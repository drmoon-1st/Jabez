...
def tavolsagSzurt(a, b):
    ...
    try:
        ...
        # only iterate over indices present in both lists and cap at 50
        max_index = min(50, len(a), len(b))
        for x in range(max_index):
            # only consider first 16 indices for this weighted scheme
            if x < 16 and b[x] != 0:
                weight = sulyozotttSzamitas[int(x / 2)] if int(x / 2) < len(sulyozotttSzamitas) else 1
                tav += ((a[x] - b[x]) * weight) ** 2
        return tav ** 0.5
    except Exception:
        # If inputs are malformed, return a large distance so candidate is de-prioritized
        traceback.print_exc()
        return float('inf')
