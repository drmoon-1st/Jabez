...
# OpenPose parameters
params = {
    "model_folder": "/elteik_openposeapi/openpose/models",
    "model_pose" : "COCO",
...
def dontes():
    ...
    global pontok
    global K
    # Defensive: if centroidok or pontok are empty, return 0 (safe default)
    almafa = []
    for a in pontok:
        # ensure a has two components
        if isinstance(a, (list, tuple)) and len(a) >= 2:
            almafa.append(a[0])
            almafa.append(a[1])

    if not centroidok or not almafa:
        return 0

    mini = 0
    # limit K to available centroids
    k_lim = min(K, len(centroidok))
...
def tavolsagSzurt(a, b):
    # Defensive distance calculation: ensure indices exist and avoid IndexError
    try:
        tav = 0.0
        sulyozotttSzamitas = [1, 1, 3, 40, 50, 3, 40, 50]
        max_index = min(50, len(a), len(b))
        for x in range(max_index):
            if x < 16 and b[x] != 0:
                weight = sulyozotttSzamitas[int(x / 2)] if int(x / 2) < len(sulyozotttSzamitas) else 1
                tav += ((a[x] - b[x]) * weight) ** 2
        return tav ** 0.5
    except Exception:
        traceback.print_exc()
        return float('inf')
