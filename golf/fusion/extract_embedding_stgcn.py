...
def parse_args():
    parser = argparse.ArgumentParser(
        description="ST-GCN 임베딩 추출 (mmAction2, train/valid/test 지원)"
    )
    ...
    parser.add_argument('--train-pkl',  type=Path,
                        help='생성된 train PKL 파일 경로')
    parser.add_argument('--valid-pkl',  type=Path,
                        help='생성된 valid PKL 파일 경로')
    parser.add_argument('--test-pkl',   type=Path,
                        help='생성된 test PKL 파일 경로 (optional)')
    parser.add_argument('--num-workers',type=int,   default=0,
                        help='DataLoader num_workers')
    return parser.parse_args()


def extract(pkl_path: Path, split_tag: str, cfg_file: str, ckpt: str,
            device: str, out_dir: Path, num_workers: int):
    # Config 로드 및 테스트 데이터로더 설정
    cfg = Config.fromfile(cfg_file)
    # 항상 test loader 사용, ann_file = pkl_path
...
def main():
    ...
    if args.test_pkl:
        ...
        if not args.train_pkl or not args.valid_pkl:
            raise ValueError('train_pkl 또는 valid_pkl 경로를 지정하세요.')
